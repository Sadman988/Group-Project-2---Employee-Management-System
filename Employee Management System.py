...
class Utils:
    ...
    S3_BUCKET_NAME = '' #Removed for security purposes 
    SHARED_EMAIL = "" #Removed for security purposes 
    SHARED_PASSWORD = "" #Removed for security purposes 
    ...
    def s3_client():
        ...
        return boto3.client(
            's3',
            aws_access_key_id='', #Removed for security purposes 
            aws_secret_access_key='' #Removed for security purposes 
        )
    ...
    def validate_email(email):
        ...
        if not re.fullmatch(r"[A-Za-z0-9]+@gmail\.com", email):
            return False
...
class EmployeeDashboard(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller, employee_data):
        ...
        tab_names = ["Home", "Edit Information", "View Jobs", "Log Out"]
        tab_frames = []
...
class EmployerDashboard(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        tab_names = [
            ...
            "Post Jobs", "Completed Jobs", "Log Out"
        ]
        tab_frames = []
        ...
        self.init_post_jobs_tab(tab_frames[3])
        self.init_completed_jobs_tab(tab_frames[4])
    ...
    def init_view_employees_tab(self, frame):
        ...
        def clear_search():
            ...
            self.filtered_employees = list(self.employees.values())  # Reset filtered_employees
            self.update_employee_listbox(employee_listbox, self.filtered_employees)
...
class EmployeeRegister(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.fields = [
            ...
            ("Email", "Enter Email Address", Utils.validate_email),
            ("Password", "Enter Password", Utils.validate_password),
        ]
    ...
    def submit(self):
        ...
        if not Utils.validate_password(self.password_entry.get()):
            messagebox.showerror("Error", "Invalid Password. Minimum 6 characters required.")
            return
